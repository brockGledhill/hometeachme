...
$getmemberinfo = mysql_query("SELECT * FROM `ward_members` WHERE MemberID='$phpmemeditid' AND WardID='$mywardid'");
...
<div id="mainbox">
    ...
    <h4 class="pagetitles">Edit Member</h4>
    	...
    	<form id="editmemform" action="updatemem.php" method="post">
        <input style="display:none;" name="thewardidname" value="<?php echo  $mywardid; ?>"/>
        <input style="display:none;" name="thememberid" value="<?php echo $row[MemberID]; ?>"/>
        
        <div class="addcompanrow">
        <span class="familytitle">First Name</span>
        <input id="firstnameid" name="firstnamename" type="text" value="<?php echo $myfirstname; ?>" />
        </div>
        <div class="addcompanrow">
        <span class="familytitle">Last Name</span>
        <input id="lastnameid" name="lastnamename" type="text" value="<?php echo $row[Last_Name]; ?>" />
        </div>
        <div class="addcompanrow">
        <span class="familytitle">Spouse</span>
        <input id="spousenameid" name="spousename" type="text" placeholder="(first name)" value="<?php echo $row[Spouse_Name]; ?>" />
        </div>
         <div class="addcompanrow">
        <span class="familytitle">Phone</span>
        <input id="phoneid" name="phonename" type="text" value="<?php echo $row[Phone]; ?>" />
        </div>
         <div class="addcompanrow">
        <span class="familytitle">Email</span>
        <input id="emailid" name="emailname" type="text" value="<?php echo $row[Email]; ?>"  />
        </div>
         <div class="addcompanrow">
        <span class="familytitle">Password</span>
        <input id="passwordid" name="passwordname" type="text" value="<?php echo $row[Password]; ?>" />
        </div>
        <div class="addcompanrow">
        ...
        <select id="jrcompid" name="jrcompname"><option value="0">No</option><option value="1">Yes</option></select>
        	...
        	<a href="javascript: updatemember(<?php echo $phpmemeditid; ?>)" class="newchangebtn btn btn-default" >Save Changes</a>
            <a href="members.php" class="canceledit btn btn-default">Cancel</a>
...
$("#jrcompid").val(<?php echo $row[Is_Jrcomp]; ?>);
